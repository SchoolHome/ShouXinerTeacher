...
//    }
    //iOS 4.3.1 Zhantai/1.0 build 11
    NSString * userAgent = [NSString stringWithFormat:@"iOS %@ palm/%@ build %@",[[UIDevice currentDevice] systemVersion],app_version,app_build];
    return userAgent;
}
...
//    url = [NSURL URLWithString:urlStr];
    ...
    self.request = [PalmHTTPRequest requestWithURL:url];
...
-(void)setHttpRequestPostWithUrl:(NSString *)urlStr params:(NSDictionary*)params{
    ...
    self.dataRequest = [PalmFormDataRequest requestWithURL:url];
...
-(void) startAsynchronous{
    if (self.dataRequest){
        [self.dataRequest setDelegate:[PalmNetWorkService sharedInstance]];
        [[PalmNetWorkService sharedInstance] startAsynchronous:self.dataRequest];
    }
    if (self.request) {
        [self.request setDelegate:[PalmNetWorkService sharedInstance]];
        [[PalmNetWorkService sharedInstance] startAsynchronous:self.request];
    }
}
